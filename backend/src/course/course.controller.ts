...
export class CourseController {
  ...
  @Get('/favorites')
  async findFavorites(@Req() req): Promise<Course[]> {
    return await this.courseService.findFavorites(req.user.userId);
  }

  @Get('/enrolled')
  async enrolledList(@Req() req): Promise<Course[]> {
    return await this.courseService.enrolledList(req.user.userId);
  }
  ...
  async addFavoriteCourse(
    ...
    return await this.courseService.addFavoriteCourse(req.user.userId, id);
  }

  @Patch('/delFavorite/:id')
  async delFavoriteCourse(@Req() req, @Param('id') id: string): Promise<void> {
    return await this.courseService.delFavoriteCourse(req.user.userId, id);
  }

  @Post('/enroll/:id')
  async enrollCourse(@Req() req, @Param('id') id: string): Promise<Course> {
    return await this.courseService.enrollCourse(req.user.userId, id);
  }

  @Patch('/unenroll/:id')
  async unenrollCourse(@Req() req, @Param('id') id: string): Promise<void> {
    return await this.courseService.unenrollCourse(req.user.userId, id);
  }
}
