...
export default function Courses() {
  ...
  const handleListFavorites = async () => {
    try {
      const favoriteCourses = await courseService.favoritesList();
    ...
    <Layout>
      ...
      {authenticatedUser.role !== 'user' ? (
        <>
          ...
          <button
            className="btn my-5 flex gap-2 w-full sm:w-auto justify-center"
            onClick={handleListFavorites} // Llama a la función que lista los favoritos
          >
            Favorites
          </button>
        </>
      ...
      <CoursesTable
        data={favoriteCourses.length > 0 ? favoriteCourses : data}
        isLoading={isLoading}
      />
