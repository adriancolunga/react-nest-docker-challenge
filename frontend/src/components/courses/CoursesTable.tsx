...
import { AlertTriangle, CheckCircle, Circle, Loader, X } from 'react-feather';
...
export default function CoursesTable({ data, isLoading }: UsersTableProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [favoritesResponse, enrolledResponse] = await Promise.all([
          courseService.favoritesList(),
          courseService.enrolledList(),
        ]);

        setFavoriteCourseIds(favoritesResponse.map((course) => course.id));
        setEnrolledCourseIds(enrolledResponse.map((course) => course.id));
      } catch (error) {
  ...
  const handleAddFavorite = async (courseId: string) => {
    try {
      await courseService.addFavorite(courseId);
      setFavoriteCourseIds((ids) => [...ids, courseId]);
    } catch (error) {
      setError(error.response.data.message);
    }
  };
  ...
  const handleRemoveFavorite = async (courseId: string) => {
    try {
      await courseService.removeFavorite(courseId);
      setFavoriteCourseIds((ids) => ids.filter((id) => id !== courseId));
    } catch (error) {
      setError(error.response.data.message);
    }
  };
  ...
  return (
    <>
      <div className="table-container">
        <Table columns={['Name', 'Description', 'Created', 'Enrolled']}>
          {isLoading
            ? null
            : data.map(({ id, name, description, dateCreated }) => (
                <tr key={id}>
                  ...
                  <TableItem className="text-right">
                    ...
                    <button
                      className={`text-green-600 hover:text-green-900 ml-3 focus:outline-none`}
                      onClick={() =>
