...
export default function ContentsTable({
  ...
  useEffect(() => {
    const fetchEnrolledStatus = async () => {
      try {
        const res =
          (await courseService.enrolledList()).filter(
            ({ id }) => id === courseId,
          ).length > 0;
        setIsEnrolled(res);
      } catch (error) {
    ...
    <>
      <div className="flex justify-between items-center mb-5">
        ...
        <div className="flex flex-col gap-3 items-start">
          {' '}
          {/* Cambiamos a flex-col para organización vertical */}
          <button
            className="btn mt-3" // Se agregó margen superior para la separación
